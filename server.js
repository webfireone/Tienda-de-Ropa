...
app.get("/api/status-servicios", async (req, res) => {
  try {
    ...
    res.json({
      github: ghToken ? { configured: true, username: "conectado" } : { configured: false, error: "GH_TOKEN no configurado" },
      vercel: vToken ? { configured: true } : { configured: false, error: "VERCEL_API_TOKEN no configurado" },
      render: rToken ? { configured: true } : { configured: false, error: "RENDER_API_KEY no configurado" },
